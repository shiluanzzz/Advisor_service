package model

type Advisor struct {
	Id             int64   `structs:"id" json:"id"`
	Name           string  `structs:"name" json:"name" validate:"required,min=4,max=20"`
	Password       string  `structs:"password" json:"password" validate:"required,min=6,max=12"`
	Phone          string  `structs:"phone" json:"phone" validate:"required,number,len=11"`
	Coin           float64 `structs:"coin" json:"coin" validate:"number"`
	TotalOrderNum  int     `structs:"total_order_num" json:"totalOrderNum" validate:"number"`
	Status         int     `structs:"status" json:"status" validate:"required,min=0,max=1"`
	Rank           float64 `structs:"rank" json:"rank" validate:"number"`
	RankNum        int     `structs:"rank_num" json:"rankNum" validate:"number"`
	WorkExperience int     `structs:"work_experience" json:"workExperience" validate:"required,number,gte=0"`
	Bio            string  `structs:"bio" json:"bio" validate:"max=50"`
	About          string  `structs:"about" json:"about"`
}
